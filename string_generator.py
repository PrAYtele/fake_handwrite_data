...
def create_strings_from_file(filename,count,lang_dict):
    strings = []
    word_labels = []
    with open(filename, 'r', encoding="utf8") as f:
        ...
        for line in lines:
            ...
            for s in line:
                try: 
                    current_label.append(lang_dict.index(s + '\n'))
                except ValueError:

                    break
            if len(current_label) != len(line):
                continue
            else:
        ...
        while len(strings) < count:
            if len(lines) >= count - len(strings):
                ...
                word_labels.extend(line_labels[0:count - len(word_labels)])
            else:
                strings.extend(lines)    
                word_labels.extend(line_labels)
    return strings,word_labels
...
def create_strings_from_dict(length, allow_variable, count, lang_dict):
    ...
    for _ in range(0, count):
        ...
        for _ in range(0, random.randint(1, length) if allow_variable else length):
            k = random.randrange(dict_len)
            # print(k)
            # print(lang_dict[k][:-1])
            current_string += lang_dict[k][:1]
            current_string += ' '
            current_label.append(k)
